...
import re
from tools.llm_agent.generation import LLMGenerationManager, GenerationConfig
...
class RayPPOTrainer(object):
    ...
    def _create_dataloader(self):
        ...
        self.val_dataset = RLHFDataset(parquet_files=self.config.data.val_files,
                                       ...
                                       return_raw_chat=self.config.data.get('return_raw_chat', False),
                                       truncation='error')
        if self.config.data.test_files is not None:
            self.test_dataset = RLHFDataset(parquet_files=self.config.data.test_files,
                                       ...
                                       filter_prompts=True,
                                       return_raw_chat=self.config.data.get('return_raw_chat', False),
                                       truncation='error') # TODO this is another evaluation dataset
        else:
    ...
    def _validate(self):
        ...
        gen_config = GenerationConfig(
            ...
            wiki_url = self.config.retriever.wiki_url,
        ...
        if not self.config.do_search:
            for test_data in self.val_dataloader:
                ...
                test_gen_batch.meta_info['data_source'] = test_batch.non_tensor_batch.get('data_source', 'unknown')
                ...
                reward_tensor, _all_metrics = self.val_reward_fn(test_batch, return_metrics=True)

                reward_tensor_lst.append(reward_tensor)
                all_metrics.extend(_all_metrics)
                data_source_lst.append(test_batch.non_tensor_batch.get('data_source', ['unknown'] * reward_tensor.shape[0]))
        else:
            dataloader_lst = [self.val_dataloader, self.test_dataloader]
            for loader in dataloader_lst:
                if loader is None:
                    continue
                for batch_dict in loader:
                    ...
                    test_gen_batch.meta_info['data_source'] = test_batch.non_tensor_batch["data_source"]
                    with _timer('step', timing_raw):
                        ...
                        with _timer('gen', timing_raw):
                            ...
                            #     print(test_gen_batch.batch['input_ids'].shape)
        ...
        data_source_reward = defaultdict(dict)
        for i, metrics in enumerate(all_metrics):
            data_source = data_sources[i]
            for metric in metrics:
                if metric not in data_source_reward[data_source]:
                    data_source_reward[data_source][metric] = []
                data_source_reward[data_source][metric].append(metrics[metric])

        metric_dict = defaultdict(dict)
        for data_source, metrics in data_source_reward.items():
            for metric, values in metrics.items():
                metric_dict[f'val/test_score/{data_source}'][metric] = np.mean(np.array(values))
        em_scores = []
        f1_scores = []
        for k, v in metric_dict.items():
            if isinstance(v, dict) and 'em' in v and 'f1' in v:
                metric_dict[k]['em'] = round(v['em'], 4)
                metric_dict[k]['f1'] = round(v['f1'], 4)
                em_scores.append(v['em'])
                f1_scores.append(v['f1'])
        
        if em_scores and f1_scores:
            metric_dict['val/test_score/average'] = {
    ...
    def fit(self):
        ...
        if self.val_reward_fn is not None and self.config.trainer.get('val_before_train', True):
            ...
            # Calculate average EM and F1 scores across all datasets
            ...
            experiment_name = self.config.trainer.experiment_name
            with open("~/InForage/data/eval_results.jsonl", 'a') as f: # TODO: change to default local dir
                import datetime
                tmp = {"name": experiment_name, "date": datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"), "metrics": val_metrics}
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                gen_batch.meta_info['source_urls'] = batch.non_tensor_batch.get('source_urls', [])
                gen_batch.meta_info['data_source'] = batch.non_tensor_batch.get('data_source', 'unknown')
                ####################
                # original code here
                ...
                # with _timer('step', timing_raw):
                    ...
                    with _timer('adv', timing_raw):
                        ...
                        step_rewards = batch.meta_info['step_rewards']

                        # we combine with rule-based rm
                        reward_tensor = self.reward_fn(batch, extra_reward=step_rewards, penalty=batch.meta_info['length_penalty'], reward_type=self.config.algorithm.reward_type)
